...
int WorkThread::recognize(const SeetaTrackingFaceInfo & faceinfo)//, std::vector<unsigned int> & datas)
{
    ...
    for(; iter != m_mainthread->m_datalst->end(); ++iter)
    {
        ...
        if(score >= gparamters.Fr_Threshold)
        {
            ...
            New_Price = 0.8*Old_Price;
            //qDebug() << "名字" << iter->second->name;
...
void WorkThread::run()
{
   ...
   while(!m_mainthread->m_exited)
   {
       ...
       datas.clear();
       errordatas.clear();
       for(unsigned int i=0; i<m_mainthread->m_mainfaceinfos.size(); i++)
       {
           if(m_mainthread->m_exited)
           {
               return;
           ...
           bfind = false;
           for(unsigned int k=0; k<m_lastpids.size(); k++)
           {
           ...
           if(!bfind)
           {
               ...
               if(nret < 0)
               {
                   ...
                   bool bsend = true;
                   for(unsigned int k=0; k<m_lasterrorpids.size(); k++)
                   {
...
void VideoCaptureThread::run()
{
    ...
    */
    ...
    while(!m_exited)
    {
        ...
        QImage imageui2((const unsigned char *)mat3.data, mat3.cols,mat3.rows,mat3.step, QImage::Format_RGB888);
        emit sigUpdateUI(imageui,imageui2);
